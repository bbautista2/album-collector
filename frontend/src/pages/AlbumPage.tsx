...
export function AlbumPage() {
  ...
  const mapStickerCandidate = (prefix: string, number: number): ScannedStickerCandidate | null => {
    ...
    const sectionPrefix = sections.find((s) => s.id === sticker!.section_id)?.prefix || ''
    const autoGeneratedName = `${sectionPrefix}${sticker.sticker_number}`
    const isAutoName = sticker.name === autoGeneratedName || sticker.name === `Sticker ${sticker.sticker_number}`
    const displayName = isAutoName ? displayCode : sticker.name
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-lg font-black text-slate-900">
                          {candidate.displayCode || `#${candidate.stickerNumber}`}
                        </p>
                      </div>
                      <input
                        type="checkbox"
                    ...
                    <p className="mt-2 truncate text-xs text-slate-700">
                      {candidate.displayName !== candidate.displayCode ? candidate.displayName : ''}
                    </p>
